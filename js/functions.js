...
const AnalyticsCookie = {
    ...
    confirmedVal: "confirmed",
    declinedVal: "declined",
    ttl: 30
};
...
*/
            ...
            if ($modal.length > 0) {
                $modal.each(function () {
                    ...
                    elem.find(modalDecline).click(function () {
                        $.cookie(AnalyticsCookie.name, AnalyticsCookie.declinedVal, {
                            expires: Number(AnalyticsCookie.ttl)
                        });
                        $.magnificPopup.close();
                        return false;
                    });
                    ...
                    if (elem.hasClass("cookie-notify")) {
                        var t = setTimeout(function () {
                            if ($.cookie(AnalyticsCookie.name) === undefined) {
                                cookieNotify.addClass(modalShowClass);
                            }
                        }, elemDelay);
                        cookieConfirm.click(function () {
                            $.cookie(AnalyticsCookie.name, AnalyticsCookie.confirmedVal, {
                                expires: Number(AnalyticsCookie.ttl)
                            });
                            w3counterVisit();
                            $.magnificPopup.close();
    ...
    if ($.cookie(AnalyticsCookie.name) === AnalyticsCookie.confirmedVal) {
        w3counterVisit();
    }
...
function w3counterVisit() {

    if (typeof window._w3counter !== 'undefined') return;

    let info = '&userAgent=' + encodeURIComponent(navigator.userAgent);
    ...
    const siteId = 147686;
    const uri = "https://www.w3counter.com/track/pv?id=" + siteId + info;

    fetch(uri, {
        mode: 'no-cors'
    })
        .catch(function (error) {
            console.log("Error: " + error + ", uri: " + uri);
        });

    window._w3counter = siteId;
}
